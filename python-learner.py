...
'''
bayesian optimization of SVM classifier parameters C and gamma
intended to be run inside the KNIME python classifier node


optimization functions adapted from
...
# Input table - data + in the last column - class (target column)

# csv file for testing from the command line
...
def bayesian_optimisation(n_iters, sample_loss, bounds, x0=None, n_pre_samples=5,
                          gp_params=None, random_search=False, alpha=1e-4, epsilon=1e-7):
...
def sample_loss(params):
    return cross_val_score(SVC(C=10 ** params[0], gamma=10 ** params[1], kernel='rbf'),
...
maxParamsIndex = np.where(yp == np.amax(yp))[0][0]


# get the best params (log10)
bestlog10C, bestlog10gamma = xp[maxParamsIndex]
bestAuroc = yp[maxParamsIndex]

bestC = 10 ** bestlog10C
bestgamma = 10 ** bestlog10gamma

print "best C", bestC
print "best gamma", bestgamma
print "best AUROC", bestAuroc


output_model = SVC(gamma=bestgamma, C=bestC, kernel='rbf', probability=True)
